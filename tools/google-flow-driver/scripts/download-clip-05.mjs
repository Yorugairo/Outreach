...
async function main() {
  ...
  const outputPath = path.join(outputDir, "clip-05-workstation-to-autolot.mp4");
  const framesDir = path.join(outputDir, "clip-05-frames");

  fs.mkdirSync(outputDir, { recursive: true });
  fs.mkdirSync(framesDir, { recursive: true });

  console.log("[download-clip-05] Locating Download scene button...");
  ...
  if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 100000) {
    console.log("Saved Clip 05 video (" + fs.statSync(outputPath).size + " bytes) to " + outputPath);
