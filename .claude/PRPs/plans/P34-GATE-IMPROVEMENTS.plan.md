...
python content/video_engine/scripts/run_script_gates.py content/video_engine/projects/systems-and-blowups/steel-and-paper/SCRIPT-G-VO.txt --ring spike --counterparty Bravos --timeline content/video_engine/projects/systems-and-blowups/steel-and-paper/build-f/timeline.json --title "The AI Bubble Is Real. What Survives Is Steel." --thumb-file content/video_engine/projects/systems-and-blowups/steel-and-paper/packaging/thumbnail-FINAL-steelpaper.png
...
opening minute rank 5/14). After E24/E25 (post-plan additions): opening
gate = 28 FAIL with the locked title (27 without `--title`, G45 then
reports INFO not-run), motion gate = 8 FAIL.
...
- CLOSE 2026-09-03 (verification re-run at the close): `pytest -k "gate or
  audit or enumerate or motion"` 220 passed / 7 failed - the seven are the
  pre-existing finance/history/remotion fixture failures named in T6, none
  in the gate suites; runner on ep1 with the locked title -> VERDICT FAIL,
  opening gate 28/1/14/7 (G45 title-word proxy FAILs on sentence 1 'The
  safest thing you own looks like this.'); motion gate on build-f -> 8 FAIL
  / 1 WARN / 3 PASS / 1 JUDGE, `GATES-MOTION.md` regenerated byte-identical
  (tracked, unchanged); `prp_validate` PASS. The rebuilt timeline
  (110 stage / 354 anchor pages, `caption_modes` declared) and the
  regenerated `SCRIPT-G-GATES.md` are committed with this close - the
  baselines above were measured on them. Memory `screen-never-still`
  carries the runner command; `REWRITE-ORDER-G.md` lines 115-126 name
  both reports as acceptance with the 28/8 baselines to beat. Fresh
  read-only review: WARNING (read-only reviewer, 2026-09-03: 1 CRITICAL, 5 HIGH, 8 MEDIUM, 5 LOW,
  no wipe/dock regression, 9 pre-existing failures outside the gate suites).
  FIXED before this close, each with a test: (1) CRITICAL - the stage caption's
  per-word pop was a wall-clock CSS transition, invisible to a scrub-and-
  screenshot render and non-deterministic; now painted inline as a function of
  t (`stagePop`, STAGE_POP_S 0.2) and the demote transition removed - stage
  innerHTML hashes equal from two seek orders at a mid-pop t (d81d2308). (2)
  HIGH - `--t0 0 --t1 <runtime>` rendered the whole episode past the gate;
  `is_full_render(args, dur)` now gates it. (3) HIGH - page beats were credited
  past the page's scene end (a 1.5s row bought 7.4s of motion); `_page_events`
  and `_species_events` clamp to the span. (4) HIGH - `--probe` skipped the
  recording refusal and the beat-tag strip; it runs both now. (5) HIGH -
  `ledger_page.validate` accepted a race file for `--variant bars` and built an
  empty page; the requested variant must have its own shape. (6) HIGH -
  `GATES-MOTION.md` had no freshness check; `write_report` writes `TIMELINE:
  <name> sha256:<hex>` and `render_episode` refuses a stale report. Also: (9)
  an unparsed tool RESULT (-1) now counts as failing; (10) M06/M07 emit INFO
  rows instead of vanishing; (12) render `--force` takes a required reason,
  appended to `render/FORCED-RENDER.md`; (16) no declared emphasis styles no
  bar. CARRIED as follow-ups (not blocking): (7) the audit defers on report
  existence, not staleness; (8) a species the player cannot resolve still
  earns a gate event; (11) G45 is opt-in without `--title` (the Verification
  command passes it); (13) M11/M12 chart-dock inference when the evidence map
  is absent; (14) `--timeline` shorter than the script truncates silently;
  (15) G28's phase check is dead; (17) `_dock_live_at` vs `_dock_span` schema;
  (18) P1/P2 tolerance overlap; the anchor caption's group punch (pre-P34,
  reviewed code) is the same wall-clock class as (1) and is left for its own
