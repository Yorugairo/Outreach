...
- Evidence: ONE-SHOT.md step 0 ends with the receipt's grammar (the two line forms, the nine stages, the ledger's last `## Recall` heading, the compile door, the `no_receipt` cost, 'a citation is not grounding'); step 9 reads 'Read the frames, then the CRITIC reads them, then hand over' with the dispatch and the card's `critic` field; RECALL-RECEIPT.md gains 'The third receipt, at the compile door - the BUILD receipt' after the second receipt (s1-s6 untouched); CAPABILITIES.md gains THE VERIFIED BUILD RECEIPT AND THE DIRECTOR-CRITIC row after the recall-receipt row; BACKLOG R26-181 BUILT (closes on HG1), R26-177's recall half DONE. Layers rebuilt and in sync.

### T7: HG1 - the calibration case: one-shot #3's receipt, verified, and the first critic pass, nothing rebuilt
- Status: done (HG1's answer owed)
- Owner: parent
...
- Evidence: (receipt half, 2026-09-17) `memory-trades-the-calendar/PRODUCTION-LEDGER.md` gains `## Recall (verified, P67 T7 - 2026-09-17)` BELOW the original thirteen lines: eighteen `Recall(<stage>)` lines across the nine stages with verbatim spans, and one paragraph naming what the 2026-09-16 pass did NOT read (the package playbook, the strip rule, the plate-production doctrine, the sound-sourcing rules, the cross-posting row; the motion doctrine only through the floor). `recall_verify.py` on the project: `Read: "## Recall (verified, P67 T7 - 2026-09-17)" at line 66 (the last of 2 headings) - 18 Recall line(s)` ... `PASS - 18 citation(s) verified across 9 stages.` (exit 0). Nothing rebuilt. (critic half, 2026-09-17) a `reviewer` dispatch read the frozen build through the served player and four probe sheets (`build-oneshot-3/critic-sheets/`) and wrote `build-oneshot-3/CRITIC.md` (untracked): `mechanisms present 10/11 (list of 2026-09-16)`, `rows attributed 10/12`. Three rows the gates never raised: the compare melt (s04 species 3) shows nothing at 29.3-30.0 s - M45 reads it `not owed`, the plan names it, the critic reads it ABSENT (FINDINGS 6: not scrub-safe); the reflection's two evidence stills have no `place` and sit ~808x597 px over the host's face (UNATTRIBUTED); the spiral page's light fires at 68.96 s inside its build window [67.82, 69.42]. The card `one-shot-3-fable-memory-calendar` now carries `critic` and one INFO line with the two fractions and the one question; `build_review_queue.py --write` / `--check` ok, the WARN now names only P66's base card.
- Evidence (the flip, 2026-09-17): P66's base got its critic (`build-p66-base/CRITIC.md`: mechanisms present 10/11, rows attributed 4/5; its first row is a compiler defect M45 read as present - the open runs 2.6 s on bare cream because the base keeps `mount=2.0` and compiles no clip row) and its card carries `critic`, the served player on :8769 and the INFO line; `CRITIC_REQUIRED = True` - an open whole-cut watch with no critic is now REFUSED by name; the fixture's whole-cut records carry a report path (the disk check is the writer's); the live-data test tolerates an empty WARN set. The queue's `--check` ok with no WARN.
