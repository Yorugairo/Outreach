...
- Evidence (the parent's read, round 4, 2026-09-17): badge-ladder READS RIGHT - a landscape card on the desk plate, no pill at 39.7 s, one at 41.4 (`META NOW $617`), two at 42.7, three at 44.0, the rail clear of the caption strip (the portrait card's rail had sat below the stage, off-screen); read-park-build-write accepted (round 3); every first tile lit. The eight: six carry their mechanism (card-becomes-the-chart, dock-lands-page-renames, plate-dock-wipe, spotlight-held-past-the-cut, read-park-build-write, badge-ladder), two are correctly `not on this bed` (test-card-rows, verdict-recap). HG2's card returns from reproof-r2 with the six as candidates and the two named with what the bed lacks; its question: do these carry the mechanisms as proven on today's bed - and does M38 return to a FAIL at 0.60, move, or stay the interim (the gates now diagnose, E99 s69). The first batch (batch-r1) re-renders through the fixed translation as batch-r2 for the parent's read before its card returns.
