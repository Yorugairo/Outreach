...
- Evidence: BLOCKED 2026-09-23 - `ev-tnx-two-eras-v3` keeps its data as two PANELS and a ledger page draws no panels (0 series; only the card chart does), and the Fed stamp is refused on it ("46 px on its long side, under the 120 px mark floor"). Unblock in flight: `ev-tnx-two-eras-v4` authored as ordinary series (the parent's call - keeps the page and the stamp); the draft is staged at `scratchpad/p69t23/door-t23-draft.py`. The Fed 6.5% / Bravos 5.5% as RULES on the yield page are E53 s5's form, not a unit mix
