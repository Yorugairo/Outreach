...
### T8: The file shapes - fetch, measure, watch, intake-triage - and the skill line on every order (2026-09-07)
- Status: complete
- Owner: parent
- Depends on: T7
- Write set: `docs/runbooks/BRIDGE-SHAPES.md` (the contract: the four shapes, the fetch manifest, the watch table, the intake-triage skeleton), `bridge_handlers.py` (`check_fetch` - manifest, hashes; `check_measure` - outputs then the tool; `check_watch` - schema verbatim, rows, required cells; `check_intake_triage` - sections, the `ours` column, status, dedupe; their templates and FORM classes), `bridge_send.py` (`--fetch-dir`, `--output`, `--csv`, `--schema`, `--required`, `--min-rows`, `--marker`, `--skill`; a `watch` order always cites `/watch` - operator 2026-09-07: "agents will not deploy the skill unless specifically cited"), tests `test_bridge_shapes.py`, the two orders `WORK-ORDER-GEMINI-BRIDGE-SHAPES-2026-09-07.md` (the convention block into the three research profiles at the source + every synced copy, marker `bridge-shapes-v1`) and `WORK-ORDER-GEMINI-WATCHER-PROFILE-2026-09-07.md` (a `video-watcher` profile that owns the `watch` shape, marker `video-watcher-v1`)
- Acceptance: the operator's asks of 2026-09-07 - "write the shapes for gemini to improve bridge report shapes ... add a Watcher Agent profile ... the intake triage skeleton request, send to gemini with request to apply globally"; each shape closes on a FILE, never on prose; the intake skeleton is clerical (the `ours` column from docs_find, status held | new | contradicts | unsourced) and the verdict stays the parent's; no evidence-density quota anywhere
- Validate: `python -m pytest content/video_engine/tests/test_bridge_shapes.py content/video_engine/tests/test_bridge_check.py content/video_engine/tests/test_bridge_handlers.py content/video_engine/tests/test_bridge_daemon.py content/video_engine/tests/test_bridge_send.py content/video_engine/tests/test_bridge_reply.py content/video_engine/tests/test_bridge_watch.py -q -p no:cacheprovider`
- Evidence: 2026-09-07 - 132 bridge tests pass (5 new: a fetch closes on a manifest whose files exist and hash, fails on an edited file (substance) or a missing field (form); a measure wants its outputs (form) then the tool (substance); a watch closes on the schema verbatim with enough rows and no blank required cell, fails on a renamed column, too few rows, a blank; an intake-triage wants the four sections (form) with the `ours` column filled and a status from the four (substance); bridge_send carries every field and names `/watch` on a watch order even when the sender forgot). Sent 03:02: packets d34755da (the shapes block into the profiles, root + marker) and 11bfd737 (the watcher profile, `--skill watch`, marker); the daemon (pid 9988) watches both. The weight order's report landed at 02:4x, failed tier 0 on `proof-line` (substance, correctly), went to tier 1 (the old daemon, before the restart): DECISION follow-up af99cf9f - four provenance defects named (a cited PDF not on disk, no URL on any proof line, two Williams quotations not in the OCR, the "1 frame" squash figure unlocated) - the follow-up is queued for the daemon to send; the report's numbers are NOT read onto the dials until the repaired report lands.
