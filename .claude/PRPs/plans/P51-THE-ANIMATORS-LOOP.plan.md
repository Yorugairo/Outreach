...
- Evidence: `samples/scene-evidence-engine.mjs` (7,688 lines, 600 KB) + the shell (564 lines, 46 KB); the engine body's diff against the old script is 48 lines in three hunks (the loader, the wrapper, the export) - `mount(doc)` with `doc || document` because the body's `"use strict"` prologue forbids a default parameter, `export { mount }` at the foot because the inliner's EXPORT_DECL did not know `async` (widened anyway); the two `window.__*` probes stay module-top-level where the old script left them. `render_baseline` (`instantiate` single-file for the goldens; `write_split`; `player_text` and `single_file_shell` for the fourteen readers that grepped the template's text); `sync_kinetics` targets the engine; `build_scene_timeline_f.main` writes the split; `.mjs` as text/javascript on `render_baseline._Quiet` and both `serve_player.py`; `prepare_page` waits for `__mounted`. Tokyo side build: player.html 32,101,827 -> 45,985 bytes, assets.json 31,407,778, the engine copy, player.json with the sha; the gate rows identical before and after; the probe's 58.6 JSON identical (the page sha aside); the self-watch the same bar; the compiled timeline and every other artifact byte-identical to the T0 baseline; `measure_page_boxes --write` re-measured every box unchanged (the sha line only). Tests: the parity pair (ledger-page-mid-build, chip-board) and the size test; every golden byte-identical (22 surfaces + 6 flag frames). The parent's run: sync 22 modules; node 215/215; 310 across goldens, page performs, sync, portrait parity, probe, self-watch, camera, breakthrough, species, lint, kit, gate. Found, not fixed: the F door's own `assert CAPTION_JS_OLD in html` fails against the pre-split template too (ep1's legacy door is already broken on its own); two pre-existing test failures named in the report.
...
- Status: complete (2026-09-11, gate 1 granted on the fourth form) - the road there: the operator's first read (2026-09-11 afternoon) named two things, both rows now - (1) the bar PASSED what it should have refused: the anchor caption under a card at 12 CSS px with no punch. Ruled E62 (the caption keeps its size and MOVES; the quiet floor 48 px / 800); the row: O8 reads E62 (a shrink under a card is a FAIL unless no band fits) and the compiler writes `caption_band` on the dock entry - SHIPPED the same day (`caption_band` / `stamp_caption_bands`, the engine's `capBand`, the 9:16 quiet floor 48 px / 800, `tests/test_caption_band.py`, `dock-pair-9x16` regenerated on purpose; Measured on Tokyo (its pages ESTIMATED - the fixture has no dense-line 9:16 entry for this ink): 4 of the 6 dock windows keep the stage caption in the home strip (y 1297, clear of the card) - at 0:38 the caption under the two-fingers card reads 66 px / 23.7 CSS px, 619 px clear of the card, where gate 1 read 33 px / 12.1; 2 windows fall to the anchor (0:57: the fab card runs into the strip and the band under the plot is 42 px, so the quiet anchor at 48 px / 17.4 CSS px); M25 PASS over 61 instants, the caption no longer the smallest type on the page; the compiled timeline gains the 6 `caption_band` fields and nothing else (stripped, equal to the pre-E62 baseline). Two things to judge: under a card the short's caption is now the full stage caption with the phrase mode's keyword punch (the anchor used to lay the whole page at once); the move between bands is instant on the dock's enter clock, not eased (a wall-clock ease never lands in a captured frame).). (2) the report cannot be judged by reading. The row: the runner writes `SELF-WATCH.html` beside the .md - the verdict first, then each O-row as one plain question a viewer can answer, what PASS and FAIL look like, the agent's read, and the frame at every instant the read names, each a link to the served player at that second (`?t=`, T4); `--html` regenerates it from the filled .md. Shipped 2026-09-11 (`self_watch.py` +267, `tests/test_self_watch.py` 12; the Tokyo copy 7 MB with 52 tiles, sent). The operator's SECOND read (2026-09-11, the HTML form beside the served build: "looks pretty good to me, maybe i'm just not seeing some of the issues" - then three things the bar had passed or flagged too softly): (3) the panel card reads over the plot while the line builds (O6 had flagged it as a question; the operator ruled it - E63, a compiler rule + M27); (4) the 0:50 transformation "basically just cuts a new chart" (no O-row asked it - ruled E64; a row for O9: a chart that becomes another chart re-writes or morphs, the hand-over named); (5) "the random back dip to black at 1:01" - the page mount's wash over the previous page and the caption dimming under it (O5 had passed the opening only - the bar reads 0:60 on a short and the defect is at 1:01; R26-50). Rows: O6 reads M27 (on the ink, since E65) and M28 (the page's own labels); O9 gains the hand-over question; O5's window on a short widens to the whole runtime for the mount / exit class; O7 asks whether any two labels read as one string. All three landed the same evening (E63 58fc622 + the clock re-fit 758d745; E64 66c4220 + 5f86c98; R26-50 6545d08) and the THIRD form was sent (build-short-t0/SELF-WATCH.html, served :8746): section 1 NOT CLEAN only on the script gates' stale viewer verdict; the read O4 O5 WARN (the JPMorgan hold), the other eight PASS. The operator's THIRD read (the same evening, from the third form's frames and a build an agent had rebuilt under its link - the review-link lesson: a form ships as a FROZEN copy from now on) named three more: the value labels crashing on the Meta page (pre-existing on the approved build; R26-53 + M28, closed), the card over the chart again (E63 widened: drawing or finished; then E65: the placer always finds a place - the plot's empty room by a measured data mask, the axis band, scale; R26-51/54 closed) and the choreography 'regressed' (the agent's in-flight measured build served live - not the record). The FOURTH form (`build-short-form4`, frozen, :8747) answers all of them; section 1 NOT CLEAN only on the script gates' stale viewer verdict; O4 O5 WARN (the JPMorgan hold). **HUMAN GATE 1 GRANTED 2026-09-11 (night), on the fourth form:** "i think the warn is a good warn. we can leave it for now because i don't want to keep cutting this when it's not needed, but i can see what a better option would've been. The effects are all looking good." - the bar's O4/O5 WARN on the JPMorgan hold stands as a correct read and the Tokyo cut is left as it is (the test bed is not re-cut for it); the dip at 61.76 is left with it. T3 complete
