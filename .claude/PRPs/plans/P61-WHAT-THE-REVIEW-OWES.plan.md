...
- Status: complete (2026-09-16) - HG2b-3 RULED E99 s62: the morph mechanism valid (the beat's second light on the crowd refused as authoring)
...
- Status: complete (2026-09-16) - HG6-2 open on the queue as `r26-76-melt-endings-in-motion` (watch, three clips)
- Owner: `implementation_luna` (**ENGINE LOCK**)
...
- Validate: `node --test content/video_engine/tests/kinetics/*.test.mjs` then `python content/video_engine/scripts/sync_kinetics.py --check` then
  ...
  `python content/video_engine/scripts/effects_catalog_check.py` then
  `python content/video_engine/scripts/review_queue_proofs.py --clips --only r26-76-melt-endings-in-motion`
- Evidence: report `scratchpad/assembly/P61-T6b.md`. MEASURED FIRST: the splash's `st.xf` was `{0, 0}` on every ending frame
  (`species/melt.mjs:1129`) - the ball burst on its seat (747.79, 882.45); the bounce and roll the operator saw were the AUTHORED
  `weight` phase (`meltWeightShare` :508 is 0 otherwise), not an insertion. BUILT: `kinetics/stopaction.mjs` `pickUpXf` (landXf
  run backwards, PRESS_PX 6 - the press into the board, then the lift) and a PITCH phase in melt.mjs: T_S 0.77 = PK_S 0.32 (the
  pick-up) + FL_S 0.45 (the flight), T_AT [0.5, 0.5] - the landing at the canvas's centre (1056, 594), T_ARC 0.18, T_LIFT_R 0.55;
  the shadow lags LAG_FRAMES behind; the splat and its stains grow from the impact point; chords 422 px (splash:chart) and 566 px
  (the gather's plate); every other phase keeps its seconds and MELT.S is untouched, so the approved instants stand. Parent's read
  of the frames: the ball pressing and lifting with its shadow tightening; in the air, stretched along the arc, the shadow left
  on the board behind it; the splat opening where it lands. GOLDENS: 4 moved (`melt-splash`, `melt-plate`, `melt-gather@proof-splash`,
  `melt-gather@proof-plate`), 6 new (`melt-splash@proof-pickup/flight/splat`, `melt-gather@proof-pickup/flight/splat`, in
  PROOF_FRAMES); every non-splash golden byte-identical. Tests: the centre rises before it advances, advances monotonically, the
  first stain's centre is the landing, the shadow trails, two seeks identical (test_melt_gather; melt.test.mjs). Validate: node
  610/610; sync in sync; pytest 297 passed; effects_catalog_check 0; build_effects_catalog STALE not INVALID (regenerated by the
  parent); page-boxes 44 (sha-only); gate on the Japan cut unchanged. NAMED: no compiler twin for T_S (MELT_T_S beside MELT_S /
  MELT_W_S / MELT_G_S - a one-line compiler follow-up); `melt:weight` before a splash still rolls AWAY from the landing as a
  wind-up (on the card); CAPABILITIES :37 / :75 want the throw written in (T11).
...
- Write set: the split's planes for `world-tokyo-customs-dock-v1` (`tests/golden/inputs/dock-layers/*-mid/-near/-subject.png` and the plate library's
  own copies), `world-tokyo-customs-dock-v1-alive.layers.json` + `.layers.json` of the flat plate if the paths move, the plate intake script that
  carries the despill (find it - `docs_find.py "despill"`, `"matte"`, `"fringe"`, the P58 T1 notes, `build_plate_library.py`), `tests/golden/frames/plate-drift.png`
  + `plate-alive.png` + their sources (20 px), `build_scene_timeline_f.py` ONLY for the named long-form setting's comment / refusal text (20),
  `build-p61-alive/` (the beat at 20; a second timeline at 30 for the card), `effects/cards/idle.json` (the setting), CAPABILITIES the alive-plate row
- Acceptance: E99 s63 verbatim - *"the processing we had done on the lamp previously had removed the green, now it's back in the ask, why? also 30 px drift
  might still be too much, maybe 20 px drift"*. The lamp's rim measured (the mean hue / chroma of the shade's 3 px edge band against the sky) on the
  raw plane, the processed plane and the flat approved plate - the processed plane matches the flat plate; the alive plate and the flat layered
