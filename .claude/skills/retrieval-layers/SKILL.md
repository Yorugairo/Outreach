...
- **Retrieval order — `python content/video_engine/scripts/docs_find.py "<term>"` first** (one compact line per hit across the layers, cheapest first; add `--layer manifest|index|topics|cites|gates|animation|craft` to focus, `--limit N` to widen), then `sed -n` the window it names. The raw `rg` per layer below is the fallback when you want the whole record (the layers are BUILD OUTPUT, gitignored since P63: `docs_find.py` rebuilds a stale one by input digest before it reads; `build_docs_layers.py --check` is a one-second digest comparison, `--ensure` the incremental rebuild, `--write` a full pass):
