...
- **Retrieval order — `python content/video_engine/scripts/docs_find.py "<term>"` first** (one compact line per hit across the layers, cheapest first; add `--layer manifest|index|topics|cites|gates|animation|craft` to focus, `--limit N` to widen), then `sed -n` the window it names. The raw `rg` per layer below is the fallback when you want the whole record (the layers are BUILD OUTPUT, gitignored since P63: a read NEVER rebuilds - the Edit/Write hook refreshes in the background and `docs_find.py` prints one stderr line if a layer is still stale; `docs_find.py --wait` when you must be fresh; `build_docs_layers.py --status` shows the refresh, `--refresh` starts one, `--check` is a one-second digest comparison, `--write` a full pass):
