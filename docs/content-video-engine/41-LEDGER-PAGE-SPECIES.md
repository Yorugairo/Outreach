...
| 3 | The field | 1.5 – 3.9s | **DECIDED (E22 addendum 4): the deckle is the feature.** The inked plate (`page.field_plate`, made procedurally from the blank page: charcoal fills the paper up to its deckle edge, 94% so a whisper of fibre survives) cross-fades over the plain page; the deckle edge appears only as the ink arrives. **E99 s35 (2026-09-15, P61 T4b): the field is authored by the sentence's JOB** - the two-plate cross-fade (`;field=plates`) when the beat maintains continuity, connects ideas or speaks across plates; the soak (`;field=soak`, filling the board box) when it builds or introduces an idea or separates two; both first-class (`page_enter:field_plates`, `page_enter:field_soak`); the scribble (`;field=scribble`) an opt-in back-up, never a default. The LEAVE is the soak's recede for both (`page_exit:retract`, `lpPlateRecede`) |
