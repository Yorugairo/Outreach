...
| 8 | **Render** | **`samples/scene-evidence-player.template.html`** | a `scene_evidence_timeline.v1` + base64 asset map | a self-contained preview **Then the self-watch (P51 T3):** `self_watch.py <build> --project <dir> --script <stem>` runs last in every build - the probe, the gate with M25, the lint, the viewer's and the script gates' verdicts, the opening's contact sheets (0:60 short / 3:00 long at 2 s) -> `<build>/SELF-WATCH.md`; the agent reads the sheets and fills O1-O10; the operator's watch begins at CLEAN. **The split (P51 T1):** the build writes `player.html` (a 46 KB shell), `assets.json` (the data URIs, fetched), a copy of `scene-evidence-engine.mjs` and `player.json` (its sha); `render_baseline.instantiate` still composes the single file for the goldens and the tests. Serve the build dir (`.mjs` as text/javascript); a `file://` open cannot fetch. |
