...
| 3–4 | **Script gates + review clearance** | `scripts/run_script_gates.py` runs the four mechanical tools; `scripts/script_review_contract.py` inventories the existing semantic duties; `scripts/script_review.py` validates the source-bound receipt and `narrative_map.v1` | script, form/stage, tool args; for required stages `--review <receipt>` + `--narrative-map <map>` + `--viewer-artifact <script-VIEWER.md>`; the receipt also binds the deterministic sibling `VIEWER-WINDOWS.json` and raw `VIEWER-REPORTS.json`; prefix/recording bind a measured `--timeline`, and recording also binds the aligned `--scratch-take` | `<script>-GATES.md` retains raw tool output and reports `MECHANICAL`, `REVIEW COMPLETENESS`, and `CLEARANCE[stage]`. Diagnostic can exit 0 while `INCOMPLETE`; no unqualified PASS exists. Required stages exit nonzero unless `CLEAR` |
| 4b | **The viewer** (P36) — the blind read | `scripts/viewer_windows.py` → `scripts/viewer_run.py` → `scripts/viewer_score.py` | exact annotated/spoken script hashes, complete window file, its timeline/screens hashes, title and FINAL thumbnail; `--require-measured` refuses estimated fallback | source-bound V01–V05 output. Missing, stale, partial or errored windows cannot enter a complete review receipt; V01 recall FAILs, confusion WARNs, gain is INFO-only |
| 5 | **Record** | `scripts/record_master_take.py`, `record_chained_take.py`, `record_short_take.py` | script plus a current `CLEARANCE[recording]: CLEAR`; preflight revalidates the receipt, map, complete viewer windows/raw reports/score chain, original/normalized measured clock and scratch-take hashes before provider work | `vo-*/audio/*.mp3` + `*.words.json`. Free-text `--force` cannot authorize recording. A future exception path must validate an operator decision, exact occurrence, source hash, scope and expiry; until then mechanical failure blocks provider work |
| 6 | **Word timeline** | `scripts/build_timeline_f.py` | the take | merged word timeline (mechanical, safe to automate) |
| 7 | **SHOT TABLE — AUTHORED** | **a human or model reading the narration** | word timeline · plate `semantic` fields · evidence `context` fields | the window table: plate + Ken Burns + docks + badge times, per beat; a plate id `ledger:<series-id>:<variant>[:<emphasize>[:<quiet_zone>]]` places a LEDGER PAGE world (doc 29 §9.26 / §9.28) that `build_scene_timeline_f.py` compiles from `evidence/objects/<series-id>.series.json` into `world.kind = "ledger"` + `world.page` (no plate PNG); an optional 7th row element `species` is a list of TARGETED SPECIES (doc 29 §9.27, P35 T7: `{"kind": punch|callout|focus_zoom|spotlight|squiggle|pull_back|plate_life|beat_freeze|radial|push, "at", "dur", "target": {"kind": datum|point|region|span, ...}}`) - the targeting law is `validate_species`: a pointing species with no declared target, two camera moves on one row, or a camera move over Ken Burns (§9.28 C3) is a hard build error naming the row; the list is emitted verbatim as `scene["species"]`; the timeline lists every species present in `species` (the ledger world + the targeted kinds) **Which species, on which sentence:** `SPECIES-BY-SENTENCE.md` (P50 T1) maps the ten sentence ACTS (QUOTES / RANKS / COMPARES / DIVIDES / NAMES / EXPLAINS / TURNS / SPANS / SETS / RETRACTS) and the widened rows (the burst E60, E58's five verbs and the park laws, the dock read->park, the camera's four reasons E59, E61's three plate uses) to the surface, the species or verb, the target it declares and the gates that read it; `SPECIES_WHEN` / `CHART_TO_WHEN` in the compiler carry the same `when` per kind; `lint_species_choice.py <project>` prints every sentence with an available species and no row (INFO) and WARNs a long-form plate with no named use (E61). |
| 7b | Motion | `scripts/build_render_f.py` | the authored table | motion plan |
| 7c | **Motion-density gate** (E21 / doc 29 §9.25) | `scripts/gate_motion_density.py` | the compiled timeline — its own `scenes[].docks` (enter / exit / badge_at) are the dock clock; `evidence-dock.json` is read only when the timeline carries no docks (P35 T0: the file drifted to an older clock) — and `motion-plan.json` cues | exit 1 on any stretch > 12s without a visual event, evidence gaps > 45s, plate holds > 20s, a thin opening minute; the list of stretches that need stage captions. A LEDGER PAGE's build beats (scene start +0 / 0.6 / 3.4 / 4.2 / 7.2s: roll-out, field, outline, build start, build complete) count as visual events and its start as an evidence entry; the hold after the build is still (§9.28 C5 / D1 / D2). Stats say `dock_source` and `ledger_pages` | Captions: the build stamps `cap_mode` per page and declares `caption_modes`; M08 FAILs a still stretch > 12s with no stage page (P34 T5). Targeted species (P35 T7) fire as events per the §9.27 "Gate treatment" column (`SPECIES_EVENTS`: punch / callout / radial / push at `at`; focus_zoom / spotlight / pull_back / beat_freeze at `at` and `at+dur`; plate_life stepping at 10 fps across its duration so it fills a bare plate; squiggle none of its own - a stage caption event only). M09 FAILs a scene that stacks two camera moves (punch | focus_zoom | pull_back) or one over Ken Burns scale > 0 - the builder rule mirrored so a hand-edited timeline is caught (§9.28 C3). Opening-minute rows (E24 / E25, §9.29–9.30): M10 FAILs any still stretch > 6s that begins in the first 60s; M11 FAILs unless the first chart (chart/data dock or ledger page) enters 0:08–0:20 with a spotlight / callout / punch / focus_zoom within 1.5s (sound cue within 1.5s or WARN); M12 FAILs a chart dock that spans a scene boundary or holds > 10s (> 6s in the opening minute) - the chart is the proof, not the homework; re-enter it spotlit. The page beats are now +0 / 0.7 / 1.5 / 3.9 / 4.7 / 5.2 / 8.2s (roll-out, savor, field, line, punch, build start, build end + focus - §9.26, E22 addendum 6). The script-side rows G45 (packaging echo, `--title`/`--thumb`), J12 (`--thumb-file`) and the G09 0:45 WARN live in stage 4's opening gate and pass through the runner. **M25 (P51 T2, the layout gate)** reads `<build>/layout-probe.json` written by `probe.py <build> --gate` (the page's own DOM at the instants `_landings` knows: every dock, page box, text size at the phone scale, overlap pair) exactly as M18 reads frame-hashes.json - INFO until measured; FAIL naming the boxes on a settled card over the chart's data, a line of the page's ink under a card, a card in the caption strip, unparked type under 11 CSS px; WARN in a safe-zone band. The self-watch (stage 8) runs the probe then the gate.
| 8 | **Render** | **`samples/scene-evidence-player.template.html`** | a `scene_evidence_timeline.v1` + base64 asset map | a self-contained preview **Then the self-watch (P51 T3):** `self_watch.py <build> --project <dir> --script <stem>` runs last in every build - the probe, the gate with M25, the lint, the viewer's and the script gates' verdicts, the opening's contact sheets (0:60 short / 3:00 long at 2 s) -> `<build>/SELF-WATCH.md`; the agent reads the sheets and fills O1-O10, then `self_watch.py <build> --html --player-url <served player>` writes `SELF-WATCH.html`, the operator's copy (one plain question per row, the frames at the instants the read names, each a link to the player at that second); the operator's watch begins at CLEAN. **After a hand edit (P51 T6):** `change_report.py <build>` -> `<build>/CHANGE-REPORT.md` - the sidecar field by field, a before/after crop pair at every instant each key touches, the gate delta, the determinism verdict; the agent reads it before the build and cites it. **The split (P51 T1):** the build writes `player.html` (a 46 KB shell), `assets.json` (the data URIs, fetched), a copy of `scene-evidence-engine.mjs` and `player.json` (its sha); `render_baseline.instantiate` still composes the single file for the goldens and the tests. Serve the build dir (`.mjs` as text/javascript); a `file://` open cannot fetch. **The publish package (R26-8, P52 T16):** the bar's last step writes `<build>/publish/` - `publish_package.py <build> [--project <dir>] [--out <dir>]` reads the build's OWN artifacts (the take's sentences in `timeline.json`, the compiled timeline's title / aspect / on-screen source lines, the project's `EVIDENCE-DOSSIER.md` Sources table and its fetch date, the URL headers of `evidence/sources/*`, the channel's `CHANNEL-DESCRIPTION.md` paste and keywords, the self-watch's 0:00 frame) and emits `DESCRIPTION-YOUTUBE.md`, `DESCRIPTION-FACEBOOK.md` (its links moved to the first comment), `PINNED-COMMENT.md`, `SOURCES.md`, `TAGS.txt`, `MANIFEST.json`, `first-frame.png`, `thumbnail.png` when the build carries one, and a one-page `CHECKLIST.md`. No hand text lives in the tool and it regenerates byte-identically; the MASTER stays the one 1440p file `render_episode.py` already writes (`RENDER_ASPECT=9:16` for a short) - the package names that file, it never renders a second size, and scheduling and API posting stay a human's (R26-8). A hand-written description for the episode (Tokyo's `SCRIPT-90S-DESCRIPTION.md`) outranks the generated one. |
