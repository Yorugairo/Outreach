...
| 8 | **Render** | **`samples/scene-evidence-player.template.html`** | a `scene_evidence_timeline.v1` + base64 asset map | a self-contained preview **Then the self-watch (P51 T3):** `self_watch.py <build> --project <dir> --script <stem>` runs last in every build - the probe, the gate with M25, the lint, the viewer's and the script gates' verdicts, the opening's contact sheets (0:60 short / 3:00 long at 2 s) -> `<build>/SELF-WATCH.md`; the agent reads the sheets and fills O1-O10; the operator's watch begins at CLEAN. |
