...
| 4b | **The viewer** (P36) — the blind read | `scripts/viewer_windows.py` → `scripts/viewer_run.py` → `scripts/viewer_score.py` | the script (+ the take's timings, the title and the FINAL thumbnail) | `<script>-VIEWER.md`: beat recall and information gain from a reader that knows no doctrine; the runner shows it as a VIEWER block, **advisory** until P36 Human Gate 1 (`--viewer-gate` promotes it) |
