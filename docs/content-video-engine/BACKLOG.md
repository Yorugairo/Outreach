...
| MM-HAND-01 | Punch-specific closed fist and wrist/forearm alignment on editable fighter rigs | 🟡 Next modeling slice | Modeling lane; trigger: opt-in contact-transfer proof and source-backed exchange style choice reviewed | Diagnose why existing Rigify finger curls still read as an open/soft palm; add a measured knuckle contact proxy, stable wrist/fist pose and needed corrective shape only behind an option. Compare jab/hook phone renders for knuckle orientation, no wrist collapse/penetration, prompt guard return and grounded body motion. Full anatomical hand simulation is not the two-day exchange gate. | [Source exchange builder](../../content/video_engine/src/modeling/blender/fight_motion.py); [exchange-first plan](../../content/video_engine/projects/martial-matters/pilots/knockout-brain-matchcut-001/production/animated-variants/exchange-first/PLAN.md) |
