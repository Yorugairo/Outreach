...
| **R26-34** | **The tip-riding pill** - a country/series pill locked to the drawing tip `X_pill(u) = P_tip(u) + D_offset` with a leader from the tip, popping on `springPop(Mp = 0.05)` at a milestone; ours names a series at its END (E53) and draws with a tip head. The bloom Gemini pairs it with (a two-pass halo + hot-core stroke) is against E22 unless ruled - the burst's glow is the one bloom we use. From Gemini's un-filed summary, technique 5. | open 2026-09-10 |
