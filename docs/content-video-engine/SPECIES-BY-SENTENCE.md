...
Rose, fell, since, monthly -> the line page with terminal tags; `build_to` the datum, `rescale` the window, `extend` the next points, `figure` at the datum; the `tiers` page (built 2026-09-11) when the same quantity is compared across two to four subjects on one x, each on its own honest scale. Row 3.

### The dividing sentence (DIVIDES a whole)
A share, a tenth, half of it -> the share page, the `peel` for the slice that leaves; the `treemap` page (built 2026-09-11) for breadth or a named subset, its cells `cross`ed on the word and the crossed share written. Row 4.

### The naming sentence (NAMES places and flows)
Crosses the border, from Ontario, the money went home -> the `vecmap` world (built 2026-09-11: a country `light`s on the word, an `arc` crosses between two, a figure `stamp`s on a place - the arc a clothoid, the light a fill never a ring); the `trace` with hops on a still. Row 5.
...
From the peak to June, the gap between two series, the run-up, the decade -> the `bracket` (its label the number, its sub the second thing), the `spread` bled full, the `span` (built 2026-09-11: a shaded stretch of time with its NAME), the `lit_stretch` (built 2026-09-24, P69 T36: a light that TRAVELS the stretch the sentence walks - the fall, the run-up - on its word, E99 s99), the `relight` on return. Row 8.

### The agenda sentence (SETS an agenda)
"Two numbers", "three things" -> the page parks (`chart_to park`) and the `agenda` takes the room it freed (built 2026-09-12: 2-4 numbered rows, each revealed on its OWN word, holding at a named idle) - or the numbered figures and the card do; `scale: 1.0` un-parks when they leave. Row 9.
...
### The counting sentence (COUNTS a set)
"Six plants", "twelve refineries", "three of them" - the sentence's weight is HOW MANY, and the count is the claim -> the `count_array` (built 2026-09-12: N identical sourced icons on a 2:1 isometric lattice, arriving in reading order one per word, the count written under them). Row 16.
...
| 3 | **COMPARES over time** - rose, fell, since, monthly, a decade | "it's been selling since February", "over the last ten years" | PAGE: the **line** page (`dense-line`), its terminal tags naming each series at its end (E53); a dense-line **chart card** for a 2-3 s beat | `build_to` to the datum the sentence reaches; `chart_to rescale` to the window the sentence is about; `extend` for "and then May"; `figure` at the datum; `recast keyed: "tags"` when the lines' end tags become the next page's bars (built 2026-09-11); `tip_pill` when the series' name should ride the pen | a datum index (`build_to`, `figure`); a window `[x0, x1]` (`rescale`); `to_index` (`extend`) | E50 (the deployed clock), M21 (the chart's life), M23 (every chart_to listed; none inside the build beat), E25 | YES (tiers page 2026-09-11) | Tokyo 20.8 *"and it's been selling since February"* -> `rescale` to the Feb-Jun window on "The Treasury's table"; 44.0 *"Since February, Japan has sold ..."* -> the bracket from the peak to June |
| 4 | **DIVIDES a whole** - a share, a tenth, half of it, the slices | "a tenth of the pile", "the share of the price that's profit" | PAGE: the **share** page (the proportion bars / the donut); the **peel** for the slice that leaves; the **treemap** for breadth or a named subset (E53 §1 census exception; P50 T6) | the slice by name (`peel`); the emphasised share | E53 §1 (a size claim takes its bar; the subset marked, its share written), E28 | share + peel YES; treemap + cross YES (2026-09-11) | Tokyo 44.0 *"a tenth of the pile"* -> the bracket's SUB carries the share (the cut chose the span over a share page: the sentence turns on the drop, the tenth is its sub) |
| 5 | **NAMES places and flows** - crosses the border, from Ontario, through the strait, the money went home | "its parts cross the border six separate times" | the **vector map** world (`vecmap:<A3 list>`: a `light` on a country, an `arc` between two, a `stamp` on a place - built 2026-09-11); a STILL with a region (the crossings map) | a country by ISO A3 (`{kind: country, id}`) or a map point (`{kind: mappoint, x, y}`); a region on the still (`trace` with `hop {from, to}`) | M18 (no frozen frames), E56 (a light, never a ring, on a picture), M14 (no camera move over a build) | vector map YES (2026-09-11); trace hops YES | tariff 13.1 *"its parts cross the border six separate times"* -> `trace` hops on `plate-gates`, six stamps stacked (25 % x3 Detroit, x2 Ontario, x1 Mexico) |
...
| 7 | **TURNS on a number** - the sentence's weight is one figure | "over a trillion dollars", "that print is your first number", "forty-five hundred dollars in tariffs" | PAGE: the **figure** stamp written at its datum; the **light** on the datum (`spotlight`, `dur: "hold"` until the sentence has a reason to leave); the **ring** on the number - `callout` for the hand's closed circle, or the `ring` kind with `form: "dashed"` for the DASHED ELLIPSE and its optional **flag chip** (P52 T8; the form widens, E56's use does not: a datum, or a label with a digit, or the compiler refuses the row) - E56; a **note** when the figure has no room at its datum | a datum index; for a note none | E56 (the ring's one use), HOLD_MIN_S (a light with < 1 s is dropped, not flashed), E25 amended (the light holds) | YES | Tokyo 23.1 *"over a trillion dollars"* -> `figure` $1,239.3B at the peak; 49.3 *"that print is your first number"* -> a `note` in the quiet zone (a figure at the June bar crossed the May bar - measured) |
| 8 | **SPANS a period or a distance** - from the peak to June; the gap between what the Fed charges and what America pays | "a hundred and twenty-two billion dollars of it", "The Fed still hasn't moved" (the distance from the low) | PAGE: the **bracket** between two data, its label the number, its sub the second thing the sentence says; the **spread** between two series, bled full of ink; the **lit_stretch** when the sentence WALKS one stretch of a drawn line (a light travels it on the word, E99 s99); the **relight** when the sentence returns to it | `from` / `to` datum indexes (bracket); `from_index` + a rule or a second series (spread); `from` / `to` x positions + `label` (span) | E52 (the bracket's label has room or it is a note), E50 (the bracket is a data mark - the clock restarts), M21 | YES (span 2026-09-11) | Tokyo 44.0 -> `bracket` "-$122.6B / a tenth of the pile" from the peak to June; 74.3 *"The Fed still hasn't moved"* -> the ring on the February low, then the `spread` from it |
| 9 | **SETS an agenda** - "two numbers", "three things", "here's what nobody says" | "Two numbers show where the money went: a Treasury page, and your phone" | PAGE parked (`chart_to park`) + the **`agenda`** (P52 T8), the numbered **figures** or the fingers card in the room the park frees. The agenda is ONE declaration of 2-4 numbered rows, each revealed on its own word (`rows[i].at`, else a word's pitch after the row before) - the number written first, the hairline drawn under the row by the nib, the text rising into place - with the block laid out for the FULL list from the first frame, so a row that arrives never pushes the one above it, and every row holding at a NAMED idle (E49) at its own phase; a **retitle** when the agenda renames the page | the block's `point` or `region` and each row's `text` / `sub` / own `at`; the park's `scale` / `anchor`; the card's box | E58 (a park moves no data, restarts no clock; it STANDS until the next park), M23, E49 (the hold breathes) | YES (the `agenda` 2026-09-12) | Tokyo 36.1 *"Two numbers show where the money went"* -> `park` 0.55 up-left on "Two", the two-fingers card lands beside it (`arrive: land, mass: metal`) |
...
| 15 | **a plate** - one of three uses, named on the row (E61) | (1) a **landing surface** - a plate thrown onto the art world with evidence docked on it (the art embed, P50 T7), or evidence docked straight onto the chart; (2) a **bridge** - a narration plate between ideas, B-roll; (3) a **reset** - a plate that covers the world so the evidence clears and the next page mounts clean or on a new topic. Every plate lives (E21, the idle) | the plate's own idle; the dip / mount at its seams (E47) | `;use=landing|bridge|reset` on the plate id. **The lint reads the token and the compiler's `PLATE_OPTS` accepts it (card `plate_option:use`); a long-form plate with no named use still WARNs** | the lint (E61: a long-form plate with no named use is a WARN), M04/M05 | tariff `plate-gates` (the crossings: a landing surface for the trace), `plate-podium` (the hook's bridge) |
| 16 | **REPORTS what was said or printed** - the wire, the headlines, the tape: "at the Treasury, Scott Bessent is issuing short-term bills", "the wire ran it all week" | the **newsreel band** (P52 T6): a STAGE species on a declared region in the lower 40 %, crawling the episode's OWN sourced headlines (the dossier's Sources, or a clip's on-screen headline - E68) against a stable upper-bound width, two copies modulo, dissolving at its right edge; the SURFACE above it is a dock - a head cutout, a clip on E45's springs, or the plate itself - and the frame is never filled with text | the band opens over 0.42 s on the house spring, the crawl starts at 0.4 s, the strap writes at 0.46 s; `hold` is the LIFE - the band stands for it and retreats over 0.38 s | `{kind: "newsreel", at, dur, headlines: [...], strap?, dateline?, speed_px_s?, hold?, cap_band?, target: {kind: "region", ...}}` on the row; the surface above is the row's own dock | the STRIP LAW (gate 1): the compiler REFUSES a row where the band and the caption's E62 band want one strip, naming both boxes - the default is the band BELOW the caption's own band, `cap_band: "above"` is the alternative; M08 / the motion gate credits a crawl as a standing element with a LIFE (one event at its arrival, continuous while it stands), never an event per frame; E68 (a clip's label is its on-screen headline; no headline is invented) | the myth of historical normal, under the Bessent sentence (SCRIPT-PRODUCTION.md's PROPOSAL block); the goldens `newsreel-band`, `newsreel-strip-9x16`, `newsreel-strip-above` |

| 16 | **COUNTS a set** - the sentence's weight is HOW MANY, and the count is the claim: "six plants", "twelve refineries", "three of them" | the **`count_array`** (P52 T7; EXPLORATION-REVIEW-2026-09-10.md:59, the reference at 7:43 - "the silos"): N identical SOURCED icons (assets/icons, A2a) on a **2:1 rhombus lattice** - cell (c, r) at x = (c - r) * PITCH, y = (c + r) * PITCH / 2, and NO perspective cheat: no vanishing point, no foreshortening, one size for every icon, nothing rotated at any t. They arrive in READING ORDER, one per word, on the chip's own two-spring landing; the COUNT is written under the field as the claim. Every cell holds at a named idle (E49) at its own phase | one icon per word (`step`, default the take's 0.34 s); the claim CLAIM_LAG after the last icon lands | `count_array {at, dur, count, icon, claim, step?, idle?}` + the `point` or `region` it is laid out in. **The compiler refuses a count the claim does not say** (`_validate_count_array`: the numeral or the word for it), a count outside 2-12, an unsourced glyph, and a field whose last icon would land after its window | E53 (the value printed), E49, M01/M10/M16 (each arrival is an event), M24 | count-array golden; the field on the `species-proof` page (P52 human gate 3) |
...
| `light` | the sentence NAMES a place - the country lights on the word, the spotlight's cousin (a fill, never a ring) |
| `arc` | the sentence NAMES a flow between two places - the arc draws from one to the other on the word; crossed when the flow is cut |
| `stamp` | the sentence puts a NUMBER or a name on a place - the figure writes at the country's centroid |
| `cross` | the sentence names a SUBSET of a census and what it adds up to - the named cells of a treemap page take an X on the word and the crossed share is written on the page (E53 s1's census exception) |
| `count_array` | the sentence COUNTS a set and the count IS the claim ('six plants', 'twelve refineries') - N identical icons arrive in reading order on an isometric field and the number is written under them |
...
Recipes per act (P56, 2026-09-13): `lint_species_choice.py <project> --propose` lists the RECIPES whose `acts` include the
sentence's act above the single-species drafts (proven first, then by count; the author binds or deletes - no allocator), and
`authoring.recipes.for_act(<ACT>)` returns the same list as data. The recipes are generated into `docs/EFFECTS-CATALOG.md`.

| species | act | task |
|---|---|---|
| beat-freeze exit, radial reveal (29 §9.27, declared but not built); the `push` species (its hand-off shipped as the press STACK, a dock kind, 2026-09-11) - all three COMPILE today with no painter (P55 T1): the catalogue lists them `declared`, `callable.today: false` | leaving a chart; a callback | open |

Built since this table was written (moved out 2026-09-13, P55 T5): the art-embed plate (P50 T7; golden `art-embed`; card `dock_option:embed`) and the continuity three (P50 T15; goldens `thread-baseline`, `occluder-dock`). Every effect by name: `python content/video_engine/scripts/effects_card.py "<name>"`.
