...
A causes B via C, "when you ..., then ..." -> the `chip` board (built 2026-09-11: a named thing as one of a set, landing on its word); the flow diagram's arrows and swap (T4) when built; a narration plate that names its use, or the page's `note`. Row 6.
