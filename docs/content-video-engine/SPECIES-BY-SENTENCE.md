...
A causes B via C, "when you ..., then ..." -> the flow diagram (T2) when built; today a narration plate that names its use, or the page's `note`. Row 6.
...
From the peak to June, the gap between two series -> the `bracket` (its label the number, its sub the second thing), the `spread` bled full, the `relight` on return. Row 8.
...
"The opponent isn't the Fed", "none of this happened" -> the `retitle` that replaces the false frame; the crossed-out board (T2) when built; a `squiggle` strike on the word. Row 10.
...
| species | act | task |
|---|---|---|
| the icon chip (a named thing as a chip on the page; the crossed-out board) | EXPLAINS, RETRACTS | P50 T2 |
