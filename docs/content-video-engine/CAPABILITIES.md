...
| **Kubelka-Munk ink** — two-flux layer model per channel (44 §44.1), K/S from the ink's own colour; the soak's stains sum coverage (`mix-blend-mode: plus-lighter`) and a per-channel table maps it through the K-M curve so overlaps DEEPEN and one full stain IS the ink; the highlighter band is a K-M layer over the chart ground | `kinetics/ink.mjs`; template soak filter + `kmHex`; flag `km_ink` | LANDED flag-off 2026-09-05; **human gate pending** (P43 T3: the 0:19-0:21 soak strip) | `ink.test.mjs` 6; rendered probe `test_kinetics_km_probe.py` within 3/255; flag golden `ledger-soak-page@km_ink` |
