...
| 3b repair | `replied/` → `queue/` | **P46 T7.** A tier-0 failure of class `form` (no grammar, a mangled or missing path list, no NOT FOUND block, stale layers, a truncated reply) queues ONE follow-up on the same conversation asking for the fill-in block, restated from what was already done - never for evidence. `repair.json` on the original is the marker; the repair packet carries `repairs: <original>`. The original waits (tier 1 skips it). The repair's reply passing tier 0 closes both (`repaired` ledger line); the repair's reply failing goes to tier 1 as the repair packet, whose `tier1` line carries `reason: <class>:<check>` - the metric. A `substance` failure (a marker absent, no proof line, an `[UNVERIFIED]` verdict, a failed `verify`) never gets a repair: a follow-up there only invites invention. |
