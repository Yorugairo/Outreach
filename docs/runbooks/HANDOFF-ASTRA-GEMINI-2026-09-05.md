...
| worker memory that persists | native `memory: user` on the read-only roles (auto-loaded ≤ 200 lines); anchors `path | heading or symbol | what`, never line numbers | see the one conflict below |
