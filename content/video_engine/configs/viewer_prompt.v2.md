...
**v2 = v1 plus the screens** (P52 T14, backlog R26-0, ruling E43: on a short the
chart carries the number, and a reader of words alone cannot see it). The only
change from v1 is that the per-part call now says what a `[screen]` line is, and
question 1 says a number read off the screen counts. `viewer_windows.py` folds
those lines in from the build; everything else is v1 word for word, so a v1 run
and a v2 run are comparable on everything except what the screens carry.
...
**What the reader is allowed to know.** Only the title, the picture, the words
it has already been given, and what is ON SCREEN while it hears them (the
`[screen]` lines). It is never told what the thing it is reading
...
Any line beginning with [screen] is not spoken. It is what is ON SCREEN in
front of you at that moment: what kind of thing it is, what it is called, and
the words and numbers printed on it. You can see those, the same way you can
hear the rest.
...
Four questions, all of them about WHAT YOU ARE HEARING NOW:
  ...
  1. What do you now know that you did not know a moment ago? List the things
     ...
     works, a claim. A number or a name you read off the screen counts just as
     much as one you heard. Say them the way you would say them to someone else, not
...
| — | — | a `[screen]` line is what you can see, not something said to you |
...
And on screen: *`[screen] a chart: "Loaves a day, Mill Street" - showing 400;
1974; 2019`*

```json
{
  "new_things": [
    "the bakery was baking 400 loaves a day by 2019, and the chart on screen shows it",
