...
def test_a_row_a_stack_and_a_quad_keep_every_panels_aspect_inside_the_region() -> None:
    for layout, k in (("row", 2), ("row", 3), ("stack", 2), ("quad", 4), ("quad", 3), ("quad", 2)):
        boxes = LPG.panel_layout(layout, k, REGION, ASPECT)
        assert len(boxes) == k
...
POSE_READ = """() => { const w = [...document.querySelectorAll('.world.ledger')].find((x) => x.__lp && x.__lp.panels);
  return w.__lp.panels.map((S) => [S.box.style.cssText, S.chart.style.opacity,
...
def test_a_focus_change_is_seek_safe_forward_play_paints_what_a_cold_seek_paints(tmp_path) -> None:
    """The quad golden mid-grow (u 0.50). Forward play in 0.1 s steps across the grow paints the cold seek's pixels
    ...
    from playwright.sync_api import sync_playwright
    tl, uris, t, aspect = RB.load_surface("panels-quad-grow")
    ...
    assert any("scale(" in p[0] and "scale(1.00000)" not in p[0] for p in poses[0]), "mid-grow, the panels are moving"
