...
MODULES = ["ease", "spring", "stagger",   # P52 T10: the caption's stagger envelope - after spring, before the caption block uses it
           ...
           "labelfit"]   # R26-191 (E99 s75): where a SPLIT value row's second line goes - inside its own bar when the bar holds it, else flush across the zero line, never a tick band; a kinetics law (no painter, no SPACE), and its region sits with the value row it serves, after record's
