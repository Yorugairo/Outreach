...
SURFACES = ["ledger-page-mid-build", "chart-callout", "dock-pair-16x9", "dock-pair-9x16", "ledger-soak-page",
            ...
            "test-card-phone",   # P69 T28b / R26-300: the same card on the checklist's `profile: "phone"` at the right 0.60 of the stage - cells, head and title at the long-form phone floor as displayed, three rows filling the canvas, read MID-FILL (row 3's paper answer 0.74 of the way through its sweep)
            ...
            "prop-stamp", "prop-stamp-ink",   # R26-20 / E99 s87: THE STAMPED PROP - one of the 24 catalogued cutouts landing BARE on a ledger page by the ported badge-stamp arrival, read 1.30 s after the contact, when both springs are at rest and the mark stands OFF-SQUARE at -8.99 deg of its -9 deg landing (a stamp never lands square). The pair differs in one byte of its source - `ink`: the woodblock in its own colour, and the same woodblock laid down in the page's own ink. Their landing instants and the owed exit ride PROOF_FRAMES
            "panels-two-eras", "panels-quad-grow",   # P69 T8b / E99 s104: the two-era panels page, and a quad whose panel 2 grows while the rest recede (u 0.50)
            "freeze-trough",   # P69 T49 / E99 s99: THE FREEZE BEAT - the railway page (H row 5, `idle=live`) stopped on its trough in the beat's held middle (12.5 s): the one light on the "-64%" datum, the spark and the drift held where they stopped (the ramps and life resuming are read by test_freeze_beat on the served player)
            "membership-builders", "membership-basket",   # P69 T45 / E99 s101: THE MEMBERSHIP STACK - one $148.4B bar divided into its five companies' EQUAL tiles (names: the catalogue carries none of their marks), the total written and "each tile = one company" beside it; and the hynix + Micron basket's two bars whose Micron tile is the catalogue's own cutout and whose SK hynix tile is its name
            "panels-mixed-grow", "bars-range",   # P69 T8d / E99 s104 amended x2: row 21's shape - a quad of two lines and two BARS panels, the wafer bars growing while the rest recede (u 0.50, the bars at Bravos's 196 px); and a bars page whose first bar is a RANGE (the bar at +55, the band to +60, "+55–60%" written, never a midpoint)
            "lit-stretch-crash",   # P69 T36 / E99 s99: THE LIT STRETCH - Steel and Paper H row 5's railway index, the light half way down the fall from the peak (53) to the trough (139) on "nearly two-thirds", its comet head at the leading edge, the rest of the line in its own ink
            "panels-resize",     # P69 T8c / E99 s104 amended: the two-era page's lone chart SHRINKING from the whole region into its row slot, re-laid out at its box's width with its words at their size, panel 2 building in beside it (u 0.50)
            "broken-axis-two-eras",   # P69 T66 / E99 s111: ONE x axis across two eras - the years between them cut and the cut DRAWN (a `//` across the axis, "2001 // 2021" written, no gridline, rule or trace through the gap), each era named over its own stretch on one years-per-pixel, one y from zero (ev-tnx-two-eras-v3's own data)
