...
def test_chart_to_chart_the_next_page_is_on_its_axes_never_empty_cream():
    """The operator, 2026-09-12: "The empty cream stage isnt supposed to be on stage during the exits ... It doesnt make
    sense to do that when transitioning from chart-to-chart, that is used for mounting a ledger plate to a narrative
    plate" - review-v1 showed cream at 34-36 s after the melt into the interest-bill page."""
    scenes = [_page_scene("s01", 1.0, 10.0, "melt", exit="cut"), _page_scene("s02", 10.0, 20.0, "cut")]
    notes = B.stamp_transition_pages(scenes)
    assert scenes[1]["world"]["page"]["enter"] == "axes"
    assert any("s02" in n and "enter=axes" in n for n in notes), notes


def test_a_plate_after_a_suck_keeps_its_own_arrival():
    """The empty cream belongs to the mount (a ledger plate onto a narrative plate): only a chart after a chart is stamped."""
    scenes = [_page_scene("s01", 1.0, 10.0, "suck", exit="cut"),
              {"scene_id": "s02", "span": [10.0, 20.0], "exit": "cut", "world": {"asset_id": "plate-x"}}]
