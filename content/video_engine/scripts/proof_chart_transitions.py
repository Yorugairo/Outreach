...
becomes its four bars by the KEYED recast at 35 s. Writes steel-and-paper/build-f/chart-transitions-proof.html
...
def main() -> int:
    ...
    scene2 = dict(tl["scenes"][0], scene_id="s02-keyed", species=species2, span=[SCENE_1_END, RUNTIME], world=dict(world2, ken_burns={"scale": 0, "x": 0, "y": 0}))
    timeline = dict(tl, aspect="9:16", runtime_s=RUNTIME, scenes=[scene, scene2], caption_pages=[], captions=[],
                    title="P48 proof: rescale 8 s, extend 13 s, park 17.5 s (the Tokyo holdings page); the keyed recast at 35 s (four lines -> four bars)")
