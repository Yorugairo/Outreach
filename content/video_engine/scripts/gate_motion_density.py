...
`*.timeline.json` (scenes, caption pages, runtime), `evidence-dock.json`
(scheduled docks) and `motion-plan.json` (cues). It is the check the
...
entering or leaving, a badge/pill reveal, or captions in STAGE mode.
Ken Burns and lower-third (anchor-mode) captions do NOT count - they are
what a viewer reads as stillness.
...
def analyse(tl: dict, docks: list[dict], mp: dict) -> dict:
    ...
    spans = [s for s in (_dock_span(d) for d in docks) if s]
    # stage-mode captions count as events when the timeline declares them
    stage_rows = [r for r in tl.get("rows", tl.get("timeline", [])) if isinstance(r, dict) and r.get("cap_mode") == "stage"]
    ...
    # evidence entry gaps, whole runtime
    entries = sorted(a for a, _ in spans)
    pts = [0.0] + entries + [runtime]
    ev_gaps = sorted(((a, b - a) for a, b in zip(pts, pts[1:])), key=lambda x: -x[1])
    # plates
    plate_ids = [s.get("world", {}).get("asset_id") for s in scenes]
    holds = [(float(s["span"][0]), float(s["span"][1]) - float(s["span"][0]), s.get("world", {}).get("asset_id")) for s in scenes]
    over_hold = []
    ...
    # per-minute density
    minutes = int(runtime // WINDOW_S) + 1
    dens = []
    ...
    return {"runtime": runtime, "events": ev, "still": still, "ev_gaps": ev_gaps, "plates": plate_ids,
            "over_hold": over_hold, "wc": wc, "pages": pages, "dens": dens, "spans": spans,
            "has_cap_mode": bool(stage_rows) or any(isinstance(r, dict) and "cap_mode" in r for r in tl.get("rows", tl.get("timeline", [])))}
...
def run(tl: dict, docks: list[dict], mp: dict) -> tuple[list[Gate], dict]:
    ...
    stats = {"runtime": mm(R), "visual_events": f"{len(A['events'])} ({len(A['events']) / (R / 60):.1f}/min)",
             "docks": len(A["spans"]), "still_over_12s_share": f"{100 * tot / R:.0f}%",
             "per_minute": " ".join(f"{mm(lo)}:{n:.0f}/{nd:.0f}" for lo, n, nd in dens)}
    return g, stats
