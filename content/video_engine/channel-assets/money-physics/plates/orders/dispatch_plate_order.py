"""Dispatch a CHANNEL plate order (an *.order.json in this directory) to the video-engine MCP server over stdio.

A copy of the tariff short's omni-video/dispatch_crossings_map.py (2026-09-08) at the channel level (P50 T7,
2026-09-11): the order file is the argument, the log and the result sit beside it, the still lands where the
order's outputPath says (channel-assets/money-physics/plates/stills/), quarantined.
...
credentials. 9222 on the default profile is only the driver's fallback.
    ...
    python content/video_engine/channel-assets/money-physics/plates/orders/dispatch_plate_order.py art-embed-study-poster.order.json

The order is FROZEN in its file. Output lands under plates/stills/, quarantined until the operator approves the frame.
"""
...
ORDER = HERE / sys.argv[1]   # the order file is the argument; a variant is its own order file
SERVER = REPO / "tools/google-flow-driver/mcp/server.mjs"
LOG = HERE / "dispatch-plate-orders.log"   # one log, every order appended
    ...
    msgs = [
        {"jsonrpc": "2.0", "id": 1, "method": "initialize",
         "params": {"protocolVersion": "2024-11-05", "capabilities": {},
                    "clientInfo": {"name": "money-physics-plate-order", "version": "1"}}},
