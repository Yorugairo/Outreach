# MOTION GATE — build-short-r51

```text
=== MOTION DENSITY GATE: C:\Users\Snipe\Downloads\Outreach Program\content\video_engine\projects\systems-and-blowups\tokyo-tea-break\build-short-r51 ===
               runtime: 1:28
  ...
  [WARN ] M27 2 card(s) inside the plot's box, clear of its ink, over 62 instants probed: dock-c-blue-ties-panel is inside the plot's box, clear of the ink, on the finished chart, at 0:09, 69,403 px, 100 % of the smaller box (marks 52% drawn) - the compiler moved this read (E63) and it still lands here; dock-g-two-fingers is inside the plot's box, clear of the ink, on the finished chart, at 0:36, 16,297 px, 13 % of the smaller box (marks 100% drawn) - E65's own placement; listed so the author sees where the read landed
  ...
  [PASS ] M25 no settled card on the chart's data, on a line of the page's ink or in the caption strip over 62 instants probed; smallest type read 11.6 CSS px on a phone (floor 11) | INFO, listed not scored (CSS px on a phone): source parked 4.9, chart.lab parked 7.5, chart.bklab parked 8.1, chart.val parked 8.1
  ...
  [PASS ] M28 no two of a page's own labels touch, and the value row keeps half a figure of air, over 62 instants probed (2,505 label pair(s) checked)
...
RESULT: 0 FAIL / 3 WARN / 18 PASS / 1 JUDGE / 4 INFO
```

TIMELINE: tokyo-short.timeline.json sha256:01c8579b152337baeae14282a6d375d92bb8db4ba2a537b1faaf309b37f98522
VERDICT: PASS (0 FAIL)
