# Publish checklist - THE HIDDEN FED METRIC BREAKING THE ECONOMY (Fed liquidity pressure) - `build-pilot`

16:9 - 2:58 - every step below is a HUMAN's. The package is written from the build's own artifacts; scheduling and API posting are out of scope (R26-8).

**The master:** `render/fed-liquidity-pilot-full-1440p.mp4` (60.2 MB) - ONE file serves YouTube, Facebook and Instagram (R26-8: the platform assigns the better codec at 1440p+ and the same upload travels). Never render a per-platform size.
...
**In this folder:** `DESCRIPTION-YOUTUBE.md`, `DESCRIPTION-FACEBOOK.md`, `PINNED-COMMENT.md`, `SOURCES.md`, `TAGS.txt`, `MANIFEST.json`, `first-frame.png`, `thumbnail.png`.

**Not on disk when this was written** (the package is thinner for it):
- the channel's own words (channel-assets/fed-liquidity-pressure/CHANNEL-DESCRIPTION.md)
