# MOTION GATE — candidate-b

```text
=== MOTION DENSITY GATE: C:\Users\Snipe\Downloads\Outreach Program\content\video_engine\projects\systems-and-blowups\fed-liquidity-pressure\build-opening-chart-proof-v1\candidate-b ===
               runtime: 0:19
         visual_events: 9 (27.8/min)
   caption_page_events: 0 from 0 page(s) (none) + 0 word arrival(s) - the stage register plus the pages a full-stage row PINNED to the anchor (R26-232)
        live_page_life: 13 event(s) every 1.613s inside s01 0:00-0:09, s02 0:09-0:19 - the PULSE rows M05/M10/M16 only (R26-232)
                 docks: 0
  ...
  [WARN ] M11 first chart ledger:s01 enters at 0.0s, its build lands at 6.0s, with spotlight at 7.0s; WARN no sound cue within 1.5s of the enter at 0.0s (no evidence species in the timeline - every dock treated as a chart candidate); window 0-10s (E44 short: the page rolls out on the hook - the chart is the mechanism by 0:10)
  ...
  [WARN ] M23 1 transition(s): s01 recast 0:07+1.6s - s01 recast at 0:07 ends inside the last 0.5 s of its page (E45: never over a build; E50: a transition is how a chart leaves, not a cut wearing a verb)
  ...
  [PASS ] M01 longest still stretch 7.0s at 0:12
          doc 29 s8.19 / s9.25 stillness ceiling
  [PASS ] M02 0 stretches > 8s (working target)
          doc 29 s9.25
  [PASS ] M03 longest wait for evidence to enter: 10s from 0:09 (no docks at all)
  ...
  [PASS ] M05 no plate over the 20s hold; TERM page_life (R26-232 (2)): the live page's own life covers 0:12+7.0s in s02, 0:03+4.0s in s01, 0:00+3.0s in s01, 0:09+3.0s in s02 - one event every 1.613s inside a page whose idle resolves to `live` (R26-228 / E99 s83: the lead point's 0.62 Hz spark and halo, the stroke's bloom pulse)
  ...
  [PASS ] M10 no still stretch > 6s begins in the first 60s; TERM page_life (R26-232 (2)): the live page's own life covers 0:12+7.0s in s02 - one event every 1.613s inside a page whose idle resolves to `live` (R26-228 / E99 s83: the lead point's 0.62 Hz spark and halo, the stroke's bloom pulse)
  ...
  [PASS ] M16 longest gap between visual events 1.6s at 0:17; 0 gap(s) over 2.5s; TERM page_life (R26-232 (2)): the live page's own life covers 0:12+7.0s in s02, 0:03+4.0s in s01, 0:00+3.0s in s01, 0:09+3.0s in s02 - one event every 1.613s inside a page whose idle resolves to `live` (R26-228 / E99 s83: the lead point's 0.62 Hz spark and halo, the stroke's bloom pulse)
  ...
  [PASS ] M21 every ledger page leaves or un-draws within 8s of its last data mark: s01 0.2s (0:09 -> 0:09); s02 5.0s (0:14 -> 0:19)
  ...
  [INFO ] M06 no caption pages in the build - M06 not run (no silent skip: build caption-pages.json first)
          s9.15 r7
  [INFO ] M07 runtime 19s has no full minute to rank - M07 not run (no silent skip)
          E21
  [INFO ] M08 timeline carries no cap_mode yet - stage captions REQUIRED on 0 stretches: 
  ...
  [JUDGE] J01 every savor beat holds its picture (card up, badge lit), never a bare plate with a drift
          doc 29 s9.25 #3
  [JUDGE] J02 captions on the still stretches above are centred, large, per-word explosive - not the lower-third anchor
          doc 29 s9.25 #2

RESULT: 0 FAIL / 2 WARN / 12 PASS / 2 JUDGE / 11 INFO
```

TIMELINE: fed-opening-chart-proof.timeline.json sha256:77aab62a71514e00d9992c4fc13b97d1fc76b4cfe7a345dd5649d57a3d5abe86
VERDICT: PASS (0 FAIL)
