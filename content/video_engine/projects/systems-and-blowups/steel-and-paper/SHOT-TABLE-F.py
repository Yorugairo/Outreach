...
( 106.0, 114.1, "world-navvy-cutting-v1",         (.05,  14,  -8), [("ev-railway-index-v1",0,102.4,111.1),("ev-railway-gdp-tile-v1",0,111.1,120.1)]),
(114.2, 120.1, "world-dotcom-server-room-v1",    (.04, -10,   6), []),  # the internet's 7% of GDP
(120.1, 141.2, "world-ledger-page-v1",           (.05,  10,  -6), [("ev-capital-formation-v1",0,120.3,168.4)]),  # "I went and pulled a version myself"
(141.4, 158.3, "world-pressure-gauge-v1",        (.05, -14,   8), []),   # the rate trigger, as an instrument
(158.3, 169.9, "world-viaduct-train-rain-v1",    (.04,  12,   6), [("ev-tnx-two-eras-v3",0,168.5,187.6)]),
(170.1, 202.0, "world-broadcast-set-v2",         (.04, -12,  -8), []),  # their line on the record, then the racetrack
...
(225.8, 231.1, "world-internal-memo-v1",         (.04,  12,   6), [("ev-doc-karp",0,225.8,236.3)]),          # Karp on CNBC
(231.1, 236.3, "world-budget-burndown-v1",       (.05, -12,   8), []),  # Uber burned the annual budget by April
(236.3, 246.7, "world-empty-racks-v1",           (.04,  10,  -8), [("ev-uber-adoption-v1",0,236.4,254.6)]),  # "can't draw a line to what you're shipping"
(247.0, 256.7, "world-exhibition-hall-morning-v1",(.05, -14,   6), [], "cut"),      # the fair the morning after = the trough
(256.8, 268.6, "hero-countercase-v1",                  (.05,  14,  -6), [("ev-three-manias",0,257.0,268.8)]),      # the trains ran through the crash  # ^REGISTER: the wave, the ruin, one green shoot - WHAT SURVIVES
...
(294.1, 300.0, "world-bond-prospectus",          (.05,  12,   6), [("ev-debt-issuance-v2",0,294.3,306.1)]),  # the borrowing, as an object
(300.1, 306.1, "world-index-board-swelling",     (.04, -10,   8), []),  # one sector crowding the index
(306.1, 325.3, "world-substation-feed-v1",       (.05,  14,  -6), [("ev-ig-credit-weighting-v1",0,306.2,325.2)]),  # money that sat in utilities
(325.3, 335.8, "world-datacenter-aisle-v1",      (.04, -12,   6), [("ev-capex-consensus-v1",0,325.9,337.8)]),
(335.8, 341.1, "world-lease-contracts-bound",    (.05, -14,   8), []),        # bound contract volumes past the frame
(341.1, 347.8, "world-datacenter-shell",         (.05,  12,  -8), [("ev-doc-leases",0,337.9,356.5)]),        # a contract pinned to the site fence
(347.8, 356.5, "beat-05-017-018-market-prices-cashflows-v1",              (.05, -10,   6), []),     # POOL: conduits pumping into the built city - every dollar consumed by the buildout
(356.5, 366.9, "beat-05-006-listed-cash-flow-market-v1",                  (.04,  10,   8), [("ev-doc-macdonald",0,356.7,374.1)]),  # POOL: what investors believe future cash flows are worth
...
(404.8, 410.4, "world-certificate-wall-v1",      (.05, -14,   8), [("ev-railway-mileage-v1",0,405.2,414.7)]),# it was railway CERTIFICATES
(410.4, 414.6, "world-exchange-floor-1845"   ,    (.05,  12,  -6), []),
(414.6, 432.2, "world-target-date-envelope-v1",  (.04, -10,   6), [("silicon-antidote-s02-valuation-bubble-v1",0,414.8,432.6)], "cut"),  # the default your retirement sits in
(432.8, 442.4, "hero-sp500-double-failure-v1",          (.05,  14,   8), [("ev-smh-drawdown-v3",0,432.8,442.9)]),   # if the names fall by half  # ^REGISTER: the index as towers cascading paper onto a crowd
(442.6, 448.8, "world-spike-certificate-ring-v2",(.06, -8,   -6), []),                       # RING TOKEN RECONTEXTUALIZED: certificate curls onto the spike
(449.8, 460.7, "world-railway-acts-desk-v1",     (.05,  10,   8), [("ev-railway-index-v1",0,450.1,460.9)]),  # 1845 is the proof
# ── P5 REFLECTION ──────────────────────────────────────────────────────
(461.2, 471.9, "world-circuit-terrain-v1",       (.05, -12,   6), [("ev-railway-mileage-v1",0,461.5,472.5)], "cut"),                       # railway steel sat 20 years; compute depreciates
...
(480.1, 515.7, "hero-korea-italy-v1",                        (.05, -10,   8), [("sovereign-memory-infrastructure-s10-teacher-stamped",0,480.1,494.6)], "cut"),  # sovereign stacks  # ^REGISTER: two continents, one hardware - sovereign stacks
(516.6, 528.8, "hero-barbell-v1",                       (.05,  12,   6), [("ev-test-scorecard-v1",0,518.4,549.0)]),  # the three questions  # ^REGISTER: the balance: gold one side, paper the other - THE TEST
...
(549.5, 558.6, "world-two-rooms-divergence-v1",  (.05, -12,   6), [("ev-divergence-v1",0,549.6,566.9)]),  # the chart RETURNS at the line that names it
...
(570.6, 583.9, "hero-fab-constraint-v1",                 (.05,  14,   8), [("ev-krx-memory-v3",0,570.9,584.9)], "cut"),     # SK hynix, +517%  # ^REGISTER: the cleanroom line - SK hynix, the constraint itself
...
(605.9, 623.6, "world-laptop-shelf",             (.04,  12,  -6), [("ev-dram-contract-v1",0,605.9,615.4),("silicon-value-software-bubble-s13-teacher-stamped",1,616.8,623.6)]),  # the blank price card
...
(670.0, 690.0, "world-unwind-desk-v2",           (.05,  10,  -8), [("ev-hbm-export-series",0,670.0,681.2)]), # THE FLIP: a position deliberately reduced - de-risked on camera
