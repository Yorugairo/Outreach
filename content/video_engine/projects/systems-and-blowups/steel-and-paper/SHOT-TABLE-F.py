...
( 104.1, 112.0, "world-navvy-cutting-v1",         (.05,  14,  -8), [("ev-railway-index-v1",0,104.1,112.0),("ev-railway-gdp-tile-v1",1,104.1,112.0)]),
(112.3, 119.3, "world-dotcom-server-room-v1",    (.04, -10,   6), []),  # the internet's 7% of GDP
...
(201.5, 215.5, "world-sell-ticket-v1"      ,     (.05,  10,   8), []),  # record earnings vs valuation - the profit-taking case
(216.1, 223.5, "world-trading-desk-dark",        (.05, -10,  -6), []),                       # "here's where their own chart gets strange"
# ── P3 GAP · unit 1: the evidence walk ─────────────────────────────────
(223.7, 228.8, "world-internal-memo-v1",         (.04,  12,   6), [("ev-doc-karp",0,223.7,233.4)]),          # Karp on CNBC
(228.8, 233.4, "world-budget-burndown-v1",       (.05, -12,   8), []),  # Uber burned the annual budget by April
(233.4, 242.8, "world-empty-racks-v1",           (.04,  10,  -8), [("ev-uber-adoption-v1",0,233.6,250.1)]),  # "can't draw a line to what you're shipping"
...
(263.4, 275.1, "world-dawn-factory-v1"  ,           (.04, -10,   8), []),  # the inescapable physical reality  # s03 carries ONE figure (97%); clears at 7.0s so the plate breathes
(275.5, 279.0, "world-molten-pour-v2",       (.05,  10,   6), []),                       # u5 rehook: "who's paying for the steel this time"
# ── P3 GAP · unit 2: the debt unit ─────────────────────────────────────
(279.1, 287.9, "world-treasury-cash-count",      (.05, -12,  -8), []),  # cash generation vs paper gains
(287.9, 292.9, "world-bond-prospectus",          (.05,  12,   6), [("ev-debt-issuance-v2",0,288.1,298.1)]),  # the borrowing, as an object
(293.0, 298.1, "world-index-board-swelling",     (.04, -10,   8), []),  # one sector crowding the index
(298.1, 314.7, "world-substation-feed-v1",       (.05,  14,  -6), [("ev-ig-credit-weighting-v1",0,298.2,314.6)]),  # money that sat in utilities
...
(439.2, 450.2, "world-railway-acts-desk-v1",     (.05,  10,   8), [("ev-railway-index-v1",0,439.4,450.4)]),  # 1845 is the proof
# ── P5 REFLECTION ──────────────────────────────────────────────────────
(450.6, 461.5, "world-circuit-terrain-v1",       (.05, -12,   6), [("ev-railway-mileage-v1",0,450.9,461.5)], "cut"),                       # railway steel sat 20 years; compute depreciates
(461.5, 466.5, "world-hbm-die-stack",                      (.04,  12,  -8), [("silicon-antidote-s11-teacher-stamped",0,461.5,466.5)], "cut"),  # orders in, slots gone  # ^REGISTER: the die stack itself - allocation sealed (ovens plate RETIRED: read as bread out of context, operator)
...
(552.3, 561.4, "world-seoul-fab-skyline-v1",     (.05, -10,   6), [], "cut"),  # the global DRAM battlefield  # extended to 9.0s so 500% / +716% can actually land
(556.0, 569.1, "hero-fab-constraint-v1",                 (.05,  14,   8), [("ev-krx-memory-v3",0,556.3,570.2)], "cut"),     # SK hynix, +517%  # ^REGISTER: the cleanroom line - SK hynix, the constraint itself
(570.4, 576.5, "hero-hbm-bandwidth-v1",                        (.06, -8,   -6), [("silicon-reality-gap-s07-hbm-stack-v1",0,570.6,576.5)]),   # dies stacked edge-on  # ^REGISTER: the die with bandwidth streaming - HBM physics
...
(589.4, 610.4, "world-laptop-shelf",             (.04,  12,  -6), [("ev-dram-contract-v1",0,589.4,598.1),("silicon-value-software-bubble-s13-teacher-stamped",1,599.3,610.4)]),  # the blank price card
(610.4, 616.5, "world-steel-mill-night",         (.05, -10,   6), [("silicon-antidote-s02-memory-triopoly-v1",0,610.4,616.5)]),  # the most vertical line is steel
...
(636.9, 651.2, "world-korea-port-v1",            (.05,  12,   6), [("ev-memory-monitor-v1",0,637.0,651.2)]),  # the monitor shows its OWN readings (s05 slide RETIRED here: no figure, partial match)
(651.2, 653.7, "world-memory-wafer-v1",          (.04, -10,   8), [("ev-trim-proof-v1",0,651.2,663.9)]),  # instrument card RETIRED (redundant with monitor analyst chart); objection card holds through the readings
(653.7, 674.5, "world-unwind-desk-v2",           (.05,  10,  -8), [("ev-hbm-export-series",1,653.7,663.9),("ev-june-print-v1",0,664.1,674.4)]), # THE FLIP: a position deliberately reduced - de-risked on camera
# ── P6 CLOSE ── re-authored to Script G's close (the beats reordered) ──
(674.5, 681.1, "world-modern-certificate-v1",    (.05, -12,   6), []),                       # certificates wear nicer names now - target-date, "the market"
(681.1, 685.4, "world-spike-certificate-ring-v2",(.05,  12,   8), []),                       # RING ECHO: the spike, one more time
(685.4, 695.9, "world-club-interior-papered",    (.05, -10,  -6), []),                       # 1850: trains still running, certificates papering bankrupt clubs
(695.9, 707.7, "beat-03-008-009-physical-capacity-gate-v1", (.04,  10,   6), []),            # everything holds - they sell scarcity, for cash; then the debt turn
(707.7, 716.0, "world-listing-barge-v1",         (.05, -10,   6), []),  # a fifth of your index - the card holds for the VERDICT beat, not here (no reuse)
(716.0, 750.5, "beat-05-002-strategic-chokepoints-v1", (.05,  14,  -6), [("ev-hynix-steel-v1",0,720.5,749.9),("ev-memory-arithmetic-v1",1,731.3,749.9)]),  # purpose-built (operator): hynix price WITH the profit under it - not a house of cards
(750.5, 763.2, "world-listing-barge-v1",         (.05, -10,   6), [("ev-index-concentration-v1",0,750.5,763.0)]),  # the verdict: the safe version IS the certificate
(763.2, 768.8, "beat-06-017-018-diworsification-v1", (.06,  10,   8), []),                   # FINAL TRIAD: steel used / paper believed / discovered at once
(768.8, 786.6, "beat-04-001-buyer-behavior-v1",  (.04, -12,   6), [("ev-test-scorecard-v1",0,768.8,773.3)]),  # CTA: you now have the test - the scorecard returns at its recap
(786.6, 795.1, "beat-06-001-003-index-product-elevator-v1", (.05,  12,  -8), []),            # future pacing: which half of your portfolio is steel
(795.1, 796.6, "world-spike-rest-v2",            (.03,   6,   4), []),                       # RING ANCHOR: the spike stays on the desk. Lamp almost out.
]
